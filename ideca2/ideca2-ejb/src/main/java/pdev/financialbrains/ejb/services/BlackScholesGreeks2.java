...
public class BlackScholesGreeks2 {
	...
	//
	private double[][] Opt ;
	public double[] calculate(String C, double s, double k, double r,
			double t, double v) {
		...
		if (C == "call") {
			...
			System.out.println("price" + p[0]);
			// delta
			p[1] = cd1;
			System.out.println("delta" + p[1]);
			// theta
			double thetaRight = r * k * exp(-r * t) * cd2;
			p[4] = thetaLeft - thetaRight;
			System.out.println("theta" + p[4]);
			// rho
			p[5] = k * t * exp(-r * t) * cd2;
			System.out.println("rho" + p[5]);
		} else {
		...
		// gamma
		p[2] = sd1 / (s * v * sqrt(t));
		System.out.println("gamma" + p[2]);
		// vega
		p[3] = s * sd1 * sqrt(t);
		System.out.println("vega" + p[3]);
		return p;
	}
