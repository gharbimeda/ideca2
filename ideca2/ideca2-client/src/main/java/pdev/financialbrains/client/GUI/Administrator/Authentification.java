...
public class Authentification extends JFrame {
	...
	public Authentification() {
		...
		lblNewLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (user instanceof BackOfficeUser) {
					...
					fermer();
					System.out.println(Util.userConnect.getIdUser());
				}
				else if (tf_login.getText().equals("admin") && tf_password.getText().equals("admin")) {
					new AdminHome().setVisible(true);
					fermer();
				}
				else{
					JOptionPane.showMessageDialog(null, " check your login or password");
				}
		...
		label_1.setIcon(new ImageIcon(
				Authentification.class.getResource("/pdev/financialbrains/client/pictures/authentif.jpg")));
